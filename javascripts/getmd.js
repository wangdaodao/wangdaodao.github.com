...
function showlist(list){
	...
	for(var i = list.data.length-(page-1)*20; i > 0 && i > list.data.length-page*20; i--){
		...
		txt += '<li><a href="'+(isroot?'':('/'+repos))+'/#!/' + encodePath(list.data[i-1].name, true) + '">' + getPostName(list.data[i-1].name) + '</a><span class="post_date">发表于 '+list.data[i-1].name.split('-')[0]+'-'+list.data[i-1].name.split('-')[1]+'-'+list.data[i-1].name.split('-')[2]+'</span></li>';
	}
	if(page==1 && page*20<list.data.length){
		txt += '<li><a class="prev_page" href="'+(isroot?'':('/'+repos))+'/#!/page/'+(page+1)+'">←较早的文章</a><div style="clear:both"></div></li>';
	}
	else if(page>1 && page*20>=list.data.length){
		txt += '<li><a class="next_page" href="'+(isroot?'':('/'+repos))+'/#!/page/'+(page-1)+'">较新的文章→</a><div style="clear:both"></div></li>';
	}
	else if(page>1 && page*20<list.data.length){
		txt += '<li><a class="prev_page" href="'+(isroot?'':('/'+repos))+'/#!/page/'+(page+1)+'">←较早的文章</a><a class="next_page" href="'+(isroot?'':('/'+repos))+'/#!/page/'+(page-1)+'">较新的文章→</a><div style="clear:both"></div></li>';
	}
	loading.style.display = 'none';
