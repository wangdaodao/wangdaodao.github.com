...
> ### 45.试着柔和的提示而不是模态窗口。  
> 模态窗口或对话框是一个独占的关注，喜欢或不喜欢。抓住用户的注意力是一件好事，但模态窗口通常有一些讨厌的问题。首先，模态窗口可以阻止用户执行其他功能或参考模态窗口后面的的信息。某些用户难以摆脱。如果用户离开计算机，对话框也经常阻止出故障的计算机而不能完成给定的工作。最后，当一个人专注于自己的工作，并且不准备采取其他行动。模态窗口可能被中断。为什么不考虑更微妙的内嵌或滑出，可能不太突出的方法同样能抓住人们的注意力？
